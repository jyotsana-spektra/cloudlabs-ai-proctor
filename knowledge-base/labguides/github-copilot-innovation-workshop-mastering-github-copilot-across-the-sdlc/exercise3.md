# Exercise 3 - Writing Unit Tests with GitHub Copilot Agent Mode
...
1. From the top menu, click **Terminal (1)**, and then select **New Terminal (2)** to open a terminal window.
...
1. Navigate to the project directory in your terminal.
   ...
   cd eCommApp
   ```
1. Run the existing test suite to ensure everything is working:

   ```bash
   npm test
   ```
1. Once the test results show Pass, stop the test and run the following command to generate a coverage report to see which parts of the code need additional testing:

   ```bash
   npm run test:coverage
   ```
1. Review the coverage report output in your terminal and note:
...
1. Open the existing test file named `CartPage.test.tsx` in the `eCommApp/src/components` directory.
...
3. Add a comment (starting with '//') stating that you'd like to test the condition where the cart displays an "empty cart message" when it is empty. 
   ...
   Please refer to the sample comment below if you get stuck.
...
5. After adding the comment, press `Enter` to go to the next line. GitHub Copilot will start suggesting lines of code, and you can press `Tab` to accept one and then press `Enter` to go to the next line, repeating this process until the test is implemented. If GitHub Copilot starts suggesting the next test, you can simply press the `Esc` key to stop the code generation process and save the file.

   ![](../../media/new/a9.png)

6. Once the test is generated, try running **npm test** to make sure that the existing test passes. If there are any failures, try asking GitHub Copilot how to fix them.

   ![](../../media/new/a10.png)

## 💭 Step 3: Using Agent Mode to Generate Additional Unit Tests

There are many other tests that we can write for `CartPage`. While we could continue using Autocomplete to generate them, that would be very slow and cumbersome. We'll instead use GitHub Copilot's Agent mode to create comprehensive unit tests.
...
1. **Open the source file**: Have `CartPage.tsx` open in a tab alongside your `CartPage.test.tsx` file.

   ![](../../media/new/a13.png)

2. **Use file references**: Include `@CartPage.tsx` in your prompts to explicitly reference the component and select **Agent** mode from **Ask** mode.
 

   ![](../../media/agent-mode.png)
...
1. Include `CartPage.tsx` in your context by opening it in a tab or referencing it in your prompt with **Agent** mode.
...
**Pro Tip:** The more specific and contextual your Agent mode prompts are, the better the generated code will be. Always review and iterate on AI-generated tests to ensure they meet your quality standards!
...
![](../../media/exc3-best-practices-prompt.png)

![](../../media/testing-quality.png)

## 🏆 Exercise Wrap-up

Great work! You've successfully used GitHub Copilot's Agent mode to:
...
1. **How did Agent mode compare to Autocomplete?**
